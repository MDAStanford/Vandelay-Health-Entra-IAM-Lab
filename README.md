# Vandelay-Health-Entra-IAM-Lab
Microsoft Entra ID lab modeling identity governance and RBAC for a fictional healthcare organization

# Vandelay Health Entra IAM Lab

## Project Overview

This project demonstrates the design and implementation of a Microsoft Entra ID environment for a fictional healthcare organization. The lab simulates identity administration and identity governance responsibilities commonly performed by IAM Analysts, including user lifecycle management, RBAC, Microsoft 365 Groups, Security Groups, executive account management, and emergency access (Break Glass) planning.

---

## Objectives

- Design a Microsoft Entra ID tenant from the ground up
- Build an organizational hierarchy
- Create workforce identities
- Implement Role-Based Access Control (RBAC)
- Configure Microsoft 365 Groups
- Configure Security Groups
- Implement a Break Glass emergency administrator account
- Document identity governance decisions
- Demonstrate IAM administration best practices

---

## Environment

| Component | Details |
|-----------|---------|
| Platform | Microsoft Entra ID |
| Organization | Vandelay Health (fictional) |
| Users | 39 |
| Departments | Executive, IT, Finance, Human Resources, Information Security, Legal & Compliance, Procurement & Supply Chain |
| Break Glass Account | 1 |
| Microsoft 365 Groups | 7 |
| Security Groups | 8 |

---

## Identity Governance Features

- Department-based identity model
- Standardized user naming convention
- Organizational manager hierarchy
- Role-based group membership
- Executive account segregation
- Break Glass emergency administrator account
- Administrative account separation
- Microsoft 365 collaboration groups
- Security Groups for authorization

---

## Repository Structure

```
docs/
    Identity-Inventory.md
    

screenshots/
    01-Entra-Dashboard.png
    02-Active-Users-1.png
    03-Active-Users-2.png
    04-Security-Groups.png
    05-M365-Groups.png
    06-CEO-Profile.png
    07-IAM-Administrator-Profile.png
    08-BreakGlass-Account.png
    09-Information-Security-Team.png

diagrams/
```

---

## Key Screenshots

- Entra Dashboard
- Active Users
- Security Groups
- Microsoft 365 Groups
- CEO Profile
- IAM Administrator Profile
- Break Glass Account
- Information Security Team

---

## Skills Demonstrated

- Microsoft Entra ID Administration
- Identity Lifecycle Management
- Role-Based Access Control (RBAC)
- Microsoft 365 Administration
- Microsoft 365 Groups
- Security Group Administration
- Identity Governance
- Break Glass Planning
- Administrative Role Assignment
- User Provisioning and Deprovisioning
- Organizational Identity Design
- Identity Documentation

---

## Future Enhancements

- Dynamic Security Groups
- Conditional Access Policies
- Privileged Identity Management (PIM)
- Access Reviews
- Entitlement Management
- Lifecycle Workflows
- Microsoft Entra Identity Protection
- SCIM Provisioning
- Hybrid Identity
- PowerShell Automation

---

## Author

**Michael Stanford**

IAM Analyst • Identity Governance • Microsoft Entra ID
